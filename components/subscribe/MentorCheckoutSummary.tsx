  ...
  return (
    <section className="rounded-2xl border border-slate-200 bg-white p-5">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <p className="text-xs font-extrabold uppercase text-slate-500">선택 멘토</p>
          <h2 className="mt-1 text-xl font-black text-slate-900">{d.displayName}</h2>
          <p className="text-xs text-slate-500">id: {props.mentorId}</p>
        </div>
        <Link
      ...
      {props.profileError ? (
        <p className="mt-2 text-sm font-bold text-amber-800">mentor_profiles: {props.profileError}</p>
      ) : null}
      <dl className="mt-4 grid gap-2 text-sm sm:grid-cols-2">
        ...
        <div>
          ...
          <dd className="font-bold text-slate-900">{d.verification || "—"}</dd>
        </div>
      </dl>
