...
export function CustomRequestCategoryGrid(props: Props) {
  ...
  return (
    <section className="space-y-2">
      ...
      {fromTable.error && !fromTable.table ? <p className="text-xs text-amber-800">categories: {fromTable.error}</p> : null}
      {fromTable.table && fromTable.error ? <p className="text-xs text-amber-800">읽기: {fromTable.error}</p> : null}
      <p className="text-xs text-slate-500">
        {fromTable.table ? `Supabase: ${fromTable.table}` : "DB 카테고리 없음 — 정적 라벨(양식 `categories` 연결 예정)."}
      </p>
