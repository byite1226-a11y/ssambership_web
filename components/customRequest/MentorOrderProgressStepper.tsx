        ...
        return (
          <li key={label} className="relative flex gap-3 pb-6 last:pb-0">
            {index < steps.length - 1 ? (
              <span
                className={`absolute left-[15px] top-8 h-[calc(100%-1rem)] w-0.5 ${done ? "bg-[#2563EB]" : "bg-slate-200"}`}
                aria-hidden
              />
            ) : null}
            <span
            ...
            <div className="min-w-0 pt-0.5">
              <p
                className={`text-sm font-extrabold ${current ? "text-[#2563EB]" : done ? "text-slate-800" : "text-slate-500"}`}
              >
                {label}
              </p>
              {current ? <p className="mt-0.5 text-[11px] font-semibold text-[#2563EB]">현재 단계</p> : null}
            </div>
          </li>
        );
      })}
