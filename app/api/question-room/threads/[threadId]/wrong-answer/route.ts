...
import { updateQuestionThreadWrongAnswerForStudent } from "@/lib/qna/questionRoomThreadService";
...
export async function PATCH(req: Request, context: RouteContext) {
  ...
  if (session.actor !== "student") {
    return NextResponse.json(
      { ok: false, error: "학생만 오답 표시를 변경할 수 있습니다." },
      { status: 403 }
    );
  }
  ...
  const isWrongAnswer = Boolean(o.isWrongAnswer);
  if (!roomId) {
    return NextResponse.json({ ok: false, error: "roomId가 필요합니다." }, { status: 400 });
  }

  const result = await updateQuestionThreadWrongAnswerForStudent(
    session.supabase,
    session.user.id,
    roomId,
    threadId.trim(),
