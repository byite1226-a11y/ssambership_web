...
export default async function LoginMentorPage(props: Props) {
  ...
  const h = loginFormHeadline.mentor;
  ...
  return (
    ...
    >
      <div className="mx-auto w-full max-w-md space-y-6 sm:space-y-7">
        <div className="rounded-3xl border-2 border-emerald-200/60 bg-emerald-50/25 p-5 shadow-[0_1px_2px_rgba(0,0,0,0.04),0_12px_32px_-8px_rgba(4,120,87,0.1)] sm:p-7">
          <Suspense fallback={<p className="text-center text-sm text-slate-500">로그인 폼을 불러오는 중…</p>}>
            <RoleLoginForm
              role="mentor"
              emailId="rl-email-ment"
              passwordId="rl-pw-ment"
              submitLabel="로그인"
        ...
        <div className="px-0.5">
          <SocialAuthButtons tone="mentor" disabled />
        </div>
        <p className="text-center text-sm text-slate-600">
          <Link href="/signup" className="font-extrabold text-emerald-800 underline decoration-emerald-200 underline-offset-4">
            멘토 회원가입
          </Link>
        </p>
      </div>
