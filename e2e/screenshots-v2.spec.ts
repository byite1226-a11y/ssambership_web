 ...
 * 저장: screenshots_v2/desktop/*.png , screenshots_v2/mobile/*.png
 * 결과: screenshots_v2/result-<viewport>.json (성공/실패/overflow)
 */
...
function publicTargets(d: Ids): Target[] {
  return [
    ...
    { name: "auth-login-admin", route: "/admin/login" },
...
for (const vp of VIEWPORTS) {
  test.describe(`screenshots-v2-${vp.tag}`, () => {
    ...
    test(`capture ${vp.tag}`, async ({ page, context }) => {
      ...
      await context.clearCookies();
      try {
        await loginAs(page, "local.admin@ssam.test", "admin");
        await captureAll(page, dir, vp, adminTargets(d));
      } catch (e) {
        failList.push(`FAIL [${vp.tag}] admin-login ${(e as Error).message.split("\n")[0]}`);
      }
      ...
      writeFileSync(join(process.cwd(), "screenshots_v2", `result-${vp.tag}.json`), JSON.stringify(summary, null, 2));
      console.log(`\n===== [${vp.tag}] OK=${summary.ok} FAIL=${summary.fail.length} SKIP=${summary.skip.length} OVERFLOW=${summary.overflow.length} =====`);
